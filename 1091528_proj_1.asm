...
.text
  ...
  main:
  	...
  	la x10, Input
  	li a7, 4	# Print string in Input
  	ecall
  	li a7, 5	# Read integer
  	ecall
  	addi x29,x0,100	#if x >=100
  	bge x10,x29,main
  	addi x9,x9,1
  ...
  End:
  	...
  	ecall
  	mv a0,a1
  	li a7, 1	#cout return result
  	ecall
  	la a0, endl	#cout a blank line
  	li a7,4
  	ecall
  	j main
 process:	
...
func2:
	...
	addi x10,x10,-2
	jal x1,condition
	sw x10,0(sp)	#store return value of f(x-2)
	lw x10,4(sp)	#讀x的值
	addi x10,x10,-3
	jal x1,condition
	lw x5,0(sp)	#load return value of f(x-3) into x5
